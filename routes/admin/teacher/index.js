...
const { getShopifyCustomers, createShopifyCustomer, getErrors } = require('../../../helpers');
...
router
  ...
  .post(async (req, res) => {
    ...
    if (customers.length === 0) {
      ...
      shopifyCustomer = await createShopifyCustomer({ data: teacher, invite: true });
    } else {
...
router
  ...
  .put((req, res) => {
    Teacher.findByIdAndUpdate(req.params.id, { $set: req.body }, function (error, result) {
      if (error) {
        let errors = getErrors(error);
        return res.status(400).send({
          errors
